...
__global__ void ccn_def_per_block(
    ...
    ref_s.load_continuous_chunk_continuous_warps(ctb, ref_mat, matrix_size.area());
    ...
    for (auto def_mat_idx = ctb.group_index().x; def_mat_idx < num_def_mats; def_mat_idx += g.group_dim().x) {
        ...
        def_s.load_continuous_chunk_continuous_warps(ctb, def_mats + def_offset, matrix_size.area());
