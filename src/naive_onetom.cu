...
__device__ void cross_corr_serial_shifts(
    cg::thread_block ctb,
    const const_matrix_slice<T>& m1,
    const const_matrix_slice<T>& m2,
    matrix_slice<RES>& out
) {
    auto warp = cg::tiled_partition<32>(ctb);
    vec2<int> half_size = (out.size - 1) / 2;
...
__global__ void ccn_def_per_block(
    ...
    for (auto def_mat_idx = ctb.group_index().x; def_mat_idx < num_def_mats; def_mat_idx += g.group_dim().x) {
        ...
        cross_corr_serial_shifts(
            ctb,
            const_matrix_slice<T>::from_position_size(
                dsize2_t{0,0},
                matrix_size,
                matrix_size.x,
                ref_s.data()
            ...
            matrix_slice<RES>::from_position_size(
                dsize2_t{0, 0},
                search_size,
                search_size.x,
                out_mats + out_offset
